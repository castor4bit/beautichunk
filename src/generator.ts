...
export class Generator {
  ...
  constructor(options: GeneratorOptions = {}) {
    this.options = {
      ...
      generateNodeEntry: options.generateNodeEntry ?? false,
    };
  }
  ...
  async generate(chunks: Chunk[]): Promise<void> {
    ...
    // Generate Node.js entry point if requested
    if (this.options.generateNodeEntry) {
      const nodeEntryContent = this.generateNodeEntry(chunks);
      const nodeEntryPath = path.join(this.options.outputDir, 'index.js');
      await fs.writeFile(nodeEntryPath, nodeEntryContent, 'utf-8');
    }
  }
  ...
  generateNodeEntry(chunks: Chunk[]): string {
    // Sort chunks by order to ensure correct execution sequence
    const sortedChunks = [...chunks].sort((a, b) => a.order - b.order);
    
    return `#!/usr/bin/env node
/**
 * Generated by Beautichunk
 * This is the entry point for the chunked Node.js application
 */

// Load all chunks in the correct order
${sortedChunks.map((chunk) => `require('./${chunk.id}.js');`).join('\n')}

// Export everything that was exported by the original module
const moduleExports = {};

// Collect all exports from global scope or module.exports
${sortedChunks
  .filter((chunk) => chunk.exports.length > 0)
  .map(
    (chunk) => `// Exports from ${chunk.id}
${chunk.exports
  .map(
    (exp) => `if (typeof ${exp} !== 'undefined') {
  moduleExports.${exp} = ${exp};
} else if (module.exports.${exp}) {
  moduleExports.${exp} = module.exports.${exp};
}`,
  )
  .join('\n')}`,
  )
...
// If this module is run directly, execute any initialization code
if (require.main === module) {
  // Original module initialization code would run here
  console.log('Beautichunk processed module loaded successfully');
}

// Export all collected exports
